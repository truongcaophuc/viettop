"use client";

import Image from "next/image";
import { motion } from "framer-motion";
import BlogSidebar from "../component/BlogSidebar"; // Assuming BlogSidebar uses t() internally
import { useTranslation, Trans } from 'react-i18next'; // 1. Import hook and Trans
import ContactDock from "@/components/layout/ContactDock";

export default function Blog6() {
  const { t } = useTranslation(); // 2. Get translation function

  const fadeInUp = {
    hidden: { opacity: 0, y: 30 },
    visible: { opacity: 1, y: 0 },
  };

  // Static data - Assuming date and author don't need translation for now
  const blogMetaData = {
    date: "15/10/2025", // Match the original component date
    author: "Thuật",
  };

  // Helper function to render content with potential HTML
  const renderContent = (contentKey: string) => {
    const translation = t(contentKey, '');
    if (!translation) return null;
    if (translation.includes('<strong')) {
      return <Trans i18nKey={contentKey} components={{ strong: <strong /> }} />;
    }
    return translation;
  };

  return (
    <>
    <section className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-10 md:gap-12 lg:gap-16"> {/* Adjusted layout */}
      {/* --- CỘT TRÁI: NỘI DUNG CHÍNH --- */}
      <motion.div
        className="space-y-6" // Adjusted spacing
        initial="hidden"
        animate="visible"
        variants={{ // Stagger children
            hidden: { opacity: 0 },
            visible: { opacity: 1, transition: { staggerChildren: 0.08, delayChildren: 0.1 } }
        }}
      >
        <motion.p
          variants={fadeInUp}
          className="text-gray-500 text-xs uppercase tracking-wider mb-1"
        >
          {t('blogPost.blogPost6.category')} {/* 3. Use t() */}
        </motion.p>

        <motion.h1
          variants={fadeInUp}
          className="text-3xl md:text-4xl font-bold text-gray-800 leading-tight"
        >
          {t('blogPost.blogPost6.title')} {/* 3. Use t() */}
        </motion.h1>

        <motion.p variants={fadeInUp} className="text-sm text-gray-500">
          {t('blogPost.blogPost6.publishedOn')}{" "}
          <span className="font-medium text-gray-700">{blogMetaData.date}</span>{" "}
          {t('blogPost.blogPost6.publishedBy')}{" "}
          <span className="font-medium text-gray-800">{blogMetaData.author}</span>
        </motion.p>

        <motion.div
          variants={fadeInUp}
          className="relative w-full aspect-video rounded-lg overflow-hidden shadow-lg group"
        >
          <Image
            src="https://congcutot.vn/uploads/store/page/article/2023/10/nghe-dien-lanh-la-nghe-gi-dd.jpg" // Consider relevant image
            alt={t('blogPost.blogPost6.bannerAlt')} // 3. Use t()
            fill
            sizes="(max-width: 1024px) 100vw, 66vw"
            className="object-cover transition-transform duration-500 group-hover:scale-105"
            priority
          />
           <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent"></div>
        </motion.div>

        {/* --- NỘI DUNG CHÍNH --- */}
        <motion.div
            variants={fadeInUp}
            className="prose prose-sm sm:prose-base max-w-none text-gray-700 leading-relaxed space-y-4 prose-headings:text-gray-800 prose-headings:font-semibold prose-h2:text-2xl prose-h3:text-xl prose-strong:text-gray-800 prose-ul:space-y-1 prose-li:my-0.5"
        >
            {/* Introduction */}
            <h2 id="intro">{t('blogPost.blogPost6.section1Title')}</h2>
            <p>{renderContent('blogPost.blogPost6.section1P1')}</p>

            {/* Centralized Management */}
            <h2 id="central-mgmt">{t('blogPost.blogPost6.section2Title')}</h2>
            <p>{renderContent('blogPost.blogPost6.section2P1')}</p>
             {/* Image 1 */}
            <div className="relative w-full aspect-video my-6 rounded-lg overflow-hidden shadow-md group">
                <Image
                    src="https://congcutot.vn/uploads/store/page/article/2023/10/nghe-dien-lanh-la-nghe-gi-dd.jpg" // Replace image
                    alt={t('blogPost.blogPost6.imageAlt1')}
                    fill
                    sizes="(max-width: 1024px) 100vw, 66vw"
                    className="object-cover transition-transform duration-500 group-hover:scale-105"
                />
            </div>

            {/* Integration Methods */}
            <h2 id="integration">{t('blogPost.blogPost6.section3Title')}</h2>
            <h3 id="integration-protocols">{t('blogPost.blogPost6.section3Sub1Title')}</h3>
            <p>{renderContent('blogPost.blogPost6.section3Sub1P1')}</p>
            <h3 id="integration-interface">{t('blogPost.blogPost6.section3Sub2Title')}</h3>
            <p>{renderContent('blogPost.blogPost6.section3Sub2P1')}</p>
            <h3 id="integration-linking">{t('blogPost.blogPost6.section3Sub3Title')}</h3>
            <p>{renderContent('blogPost.blogPost6.section3Sub3P1')}</p>
             {/* Image 2 */}
            <div className="relative w-full aspect-video my-6 rounded-lg overflow-hidden shadow-md group">
                <Image
                    src="https://congcutot.vn/uploads/store/page/article/2023/10/nghe-dien-lanh-la-nghe-gi-dd.jpg" // Replace image
                    alt={t('blogPost.blogPost6.imageAlt2')}
                    fill
                    sizes="(max-width: 1024px) 100vw, 66vw"
                    className="object-cover transition-transform duration-500 group-hover:scale-105"
                />
            </div>

            {/* Operational Benefits */}
            <h2 id="benefits">{t('blogPost.blogPost6.section4Title')}</h2>
            <ul>
                <li>{renderContent('blogPost.blogPost6.section4Item1')}</li>
                <li>{renderContent('blogPost.blogPost6.section4Item2')}</li>
                <li>{renderContent('blogPost.blogPost6.section4Item3')}</li>
                <li>{renderContent('blogPost.blogPost6.section4Item4')}</li>
            </ul>

            {/* Centralized Alerting */}
            <h2 id="alerting">{t('blogPost.blogPost6.section5Title')}</h2>
            <h3 id="alerting-priority">{t('blogPost.blogPost6.section5Sub1Title')}</h3>
            <p>{renderContent('blogPost.blogPost6.section5Sub1P1')}</p>
            <h3 id="alerting-channels">{t('blogPost.blogPost6.section5Sub2Title')}</h3>
            <p>{renderContent('blogPost.blogPost6.section5Sub2P1')}</p>
            <h3 id="alerting-correlation">{t('blogPost.blogPost6.section5Sub3Title')}</h3>
            <p>{renderContent('blogPost.blogPost6.section5Sub3P1')}</p>
             {/* Image 3 */}
            <div className="relative w-full aspect-video my-6 rounded-lg overflow-hidden shadow-md group">
                <Image
                    src="https://congcutot.vn/uploads/store/page/article/2023/10/nghe-dien-lanh-la-nghe-gi-dd.jpg" // Replace image
                    alt={t('blogPost.blogPost6.imageAlt3')}
                    fill
                    sizes="(max-width: 1024px) 100vw, 66vw"
                    className="object-cover transition-transform duration-500 group-hover:scale-105"
                />
            </div>

            {/* Conclusion */}
            <h2 id="conclusion">{t('blogPost.blogPost6.section6Title')}</h2>
            <p>{renderContent('blogPost.blogPost6.section6P1')}</p>
            <p>{renderContent('blogPost.blogPost6.section6P2')}</p>
             {/* Image 4 */}
            <div className="relative w-full aspect-video mt-6 rounded-lg overflow-hidden shadow-md group">
                <Image
                    src="https://congcutot.vn/uploads/store/page/article/2023/10/nghe-dien-lanh-la-nghe-gi-dd.jpg"
                    alt={t('blogPost.blogPost6.imageAlt4')}
                    fill
                    sizes="(max-width: 1024px) 100vw, 66vw"
                    className="object-cover transition-transform duration-500 group-hover:scale-105"
                />
            </div>

        </motion.div>
      </motion.div>

      {/* --- CỘT PHẢI: SIDEBAR --- */}
      <motion.div
        className="lg:col-span-1"
        initial={{ opacity: 0, x: 40 }}
        whileInView={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.7, delay: 0.3 }} // Adjusted delay
        viewport={{ once: true }}
      >
        <div className="sticky top-8">
           {/* Assuming BlogSidebar is translated */}
          <BlogSidebar />
        </div>
      </motion.div>
    </section>
      <ContactDock />
    </>
  );
}